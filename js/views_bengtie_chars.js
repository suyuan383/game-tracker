...
function isBtLevelMatch(a, filter) {
  const lv = +a.level || 0;
...
/* ---------- 主页面 ---------- */
function renderBtCharsTab(d) {
  const chars = d.characters || [];
  chars.forEach(c => { normalizeBtSets(c.actual); normalizeBtSets(c.recommend); });

  const charForm = (ysShowCharForm || ysEditingCharId) ? renderBtCharForm(d) : '';
  const importForm = ysShowImportForm ? renderBtImportForm() : '';
  const managePanel = renderEquipManagePanel('bengtie');

  const filtered = chars.filter(c => {
    if (!isBtLevelMatch(c.actual || {}, ysFilterLevel)) return false;
    if (ysFilterTalent !== 'all') {
      const tOk = isBtTalentOk(c.actual || {}, c.recommend || {});
  ...
  const listHtml = filtered.length
    ? filtered.map(c => renderBtCharCard(c)).join('')
    : `<div class="empty" style="padding:40px 0">没有匹配的角色</div>`;

  const allSetNames = getAllSetNames('bengtie');
  const setOptions = ['<option value="all">全部遗器</option>']
...
/* ---------- 卡片 ---------- */
function renderBtCharCard(c) {
  normalizeBtSets(c.actual); normalizeBtSets(c.recommend);
  const a = c.actual || emptyBtBuild();
  const r = c.recommend || emptyBtBuild();
  ...
  return `
    <div class="card" data-char-name="${esc(c.name)}" style="margin-bottom:10px;padding:0">
      ...
      <div style="padding:10px 14px">
        ...
        ${rowTalent()}
        ${row('光锥', a.weapon, r.weapon)}
        ${rowSets('遗器', a.sets, r.sets)}
        ${rowRec('衣服推荐', r.body)}
        ${rowRec('鞋子推荐', r.feet)}
        ${rowRec('球推荐', r.sphere)}
        ${rowRec('链子推荐', r.rope)}
...
/* ---------- 表单 ---------- */
function renderBtCharForm(d) {
  const isEditing = !!ysEditingCharId;
  let target = null;
  if (isEditing) target = (d.characters || []).find(c => c.id === ysEditingCharId);
  const a = target ? (target.actual || emptyBtBuild()) : emptyBtBuild();
  const r = target ? (target.recommend || emptyBtBuild()) : emptyBtBuild();
  normalizeBtSets(a); normalizeBtSets(r);
  ...
  const col = (prefix, build) => {
    ...
    <div style="flex:1;min-width:0">
      ...
      <div style="margin-bottom:6px"><div style="font-size:11px;color:#666;margin-bottom:2px">等级</div>
        <input class="inp" id="${prefix}_level" value="${esc(build.level)}" placeholder="如 80"></div>
      <div style="display:flex;gap:4px;margin-bottom:6px">
        ...
        <div style="flex:1"><div style="font-size:11px;color:#666;margin-bottom:2px">战技</div><input class="inp" id="${prefix}_talS" value="${esc(build.talS)}"></div>
        <div style="flex:1"><div style="font-size:11px;color:#666;margin-bottom:2px">终结技</div><input class="inp" id="${prefix}_talB" value="${esc(build.talB)}"></div>
      </div>
      <div style="margin-bottom:6px"><div style="font-size:11px;color:#666;margin-bottom:2px">光锥</div>
        <input class="inp" id="${prefix}_weapon" value="${esc(build.weapon)}"></div>

      <div style="margin-bottom:6px">
        <div style="font-size:11px;color:#666;margin-bottom:2px">遗器套装 1</div>
        <div style="display:flex;gap:4px">
          <div style="flex:2">${renderSetNameSelect('bengtie', `${prefix}_set1Name`, s1.name)}</div>
      ...
      <div style="margin-bottom:6px;${needS2?'':'display:none'}" id="${prefix}_set2Wrap">
        <div style="font-size:11px;color:#666;margin-bottom:2px">遗器套装 2（2件套）</div>
        ${renderSetNameSelect('bengtie', `${prefix}_set2Name`, s2.name)}
      </div>

      ${prefix === 'r' ? `
      <div style="margin-bottom:6px"><div style="font-size:11px;color:#666;margin-bottom:2px">衣服主属性</div>
        <input class="inp" id="${prefix}_body" value="${esc(build.body)}" placeholder="如 暴击 / 爆伤 / 生命%"></div>
      <div style="margin-bottom:6px"><div style="font-size:11px;color:#666;margin-bottom:2px">鞋子主属性</div>
        <input class="inp" id="${prefix}_feet" value="${esc(build.feet)}" placeholder="如 速度 / 攻击%"></div>
      <div style="margin-bottom:6px"><div style="font-size:11px;color:#666;margin-bottom:2px">球主属性</div>
        <input class="inp" id="${prefix}_sphere" value="${esc(build.sphere)}" placeholder="如 虚数伤害 / 攻击%"></div>
      <div style="margin-bottom:6px"><div style="font-size:11px;color:#666;margin-bottom:2px">链子主属性</div>
        <input class="inp" id="${prefix}_rope" value="${esc(build.rope)}" placeholder="如 击破特攻 / 能量恢复 / 攻击%"></div>
    ...
    <div class="card" style="margin-bottom:12px">
      ...
      <div style="display:flex;gap:8px;margin-bottom:10px">
        <div style="flex:1.6"><div style="font-size:11px;color:#666;margin-bottom:2px">角色名</div>
          <input class="inp" id="charName" value="${esc(name)}" placeholder="如 流萤"></div>
  ...
  return `
    <div class="card" style="margin-bottom:12px">
      ...
      <div style="font-size:12px;color:#666;margin-bottom:8px;line-height:1.7">
        每行一个字段，支持：<span style="color:#3b5bdb">角色名 / 等级 / 天赋：10/10/10 / 光锥 / 遗器：如 4件套 / 衣服 / 鞋子 / 球 / 链子 / 关键属性</span><br>
        用 <span style="color:#e8590c">--- 推荐 ---</span> 分隔实际与推荐
      </div>
      <textarea class="inp" id="ysImportText" style="min-height:160px;font-family:monospace;font-size:13px" placeholder="粘贴文本...">${esc(ysImportText || '')}</textarea>
...
/* ---------- 解析器 ---------- */
function parseBtBuildTextMulti(text) {
  ...
  if (markers.length >= 2) {
    ...
    return blocks.map(b => parseBtBuildText(b)).filter(c => c.name);
  }
  const rawBlocks = text.split(/\n\s*\n+/).map(s => s.trim()).filter(Boolean);
  if (rawBlocks.length > 1) return rawBlocks.map(b => parseBtBuildText(b)).filter(c => c.name);
  const single = parseBtBuildText(text);
  return single.name ? [single] : [];
}

function parseBtBuildText(text) {
  const result = { name: '', actual: emptyBtBuild(), recommend: emptyBtBuild() };
  const lines = text.split('\n').map(s => s.trim()).filter(Boolean);
  ...
  for (const line of lines) {
    ...
    parseBtLine(line, result[mode]);
  }
  return result;
}

function parseBtLine(line, target) {
  let m;
  ...
  if ((m = line.match(/^(?:光锥|武器)[：:]?\s*(.+)/))) { target.weapon = m[1].trim(); return; }
  if ((m = line.match(/^(?:遗器|圣遗物)[：:]?\s*(.+)/))) {
    target.sets = parseSetString(m[1].trim()); return;
  }
  if ((m = line.match(/^(?:衣服|躯干|身体)[：:]?\s*(.+)/))) { target.body = m[1].trim(); return; }
  if ((m = line.match(/^(?:鞋子|脚|脚部)[：:]?\s*(.+)/))) { target.feet = m[1].trim(); return; }
  if ((m = line.match(/^(?:球|位面球)[：:]?\s*(.+)/))) { target.sphere = m[1].trim(); return; }
  if ((m = line.match(/^(?:链子|连接绳|连结绳|绳)[：:]?\s*(.+)/))) { target.rope = m[1].trim(); return; }
  if ((m = line.match(/^(?:关键属性|面板|推荐属性|要求|属性)[：:]?\s*(.+)/))) { target.keyStats = m[1].trim(); return; }
}

/* ---------- 收集表单数据 ---------- */
function collectBtBuild(prefix) {
  ...
  if (s1Name === '__scatter__' || s1Name === '散件') {
    ...
    if (s1Count === 2 && s2Name && s2Name !== '__scatter__') {
      sets = [{ name: s1Name, count: 2 }, { name: s2Name, count: 2 }];
    } else if (s1Count === 2) {
  ...
  return {
    ...
    sets,
    body: get(prefix + '_body'),
    feet: get(prefix + '_feet'),
    sphere: get(prefix + '_sphere'),
    rope: get(prefix + '_rope'),
